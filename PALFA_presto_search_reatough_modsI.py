...
institution           = "MPIfR" 
#base_output_directory = cwd
#base_output_directory = "/hercules/results/aswin/results/debug/long_ddplan"
base_output_directory = "/hercules/u/aswin/results/debug"
...
# hi_accel_zmax depends on T_obs!
...
# new PMPS style plan below
ddplans.append(dedisp_plan(   0.0,   0.5,      76,     3,       96,       1))
ddplans.append(dedisp_plan( 114.0,   1.0,      76,     1,       96,       2))
ddplans.append(dedisp_plan( 190.0,   2.0,      76,     1,       96,       4))
ddplans.append(dedisp_plan( 342.0,   5.0,      66,     2,       96,       8))
ddplans.append(dedisp_plan(1002.0,  10.0,      66,     1,       96,      16))
ddplans.append(dedisp_plan(1662.0,  20.0,      66,     1,       96,      32))
#ddplans.append(dedisp_plan(2982.0,  30.0,      76,     1,       96,      64))


# test line 
#ddplans.append(dedisp_plan( 168.0,   0.5,      12,     1,       96,       1))
...
def main(fil_filenm, workdir):
    ...
    for ddplan in ddplans:
        ...
        for passnum in range(ddplan.numpasses):
            ...
            if use_subbands:
                ...
                #cmd = "prepsubband -lodm %.2f -dmstep %.2f -numdms %d -numout %d -zerodm -o %s %s"%\
            ...
            for dmstr in ddplan.dmlist[passnum]:
                ...
                print "done5"
                #os.system("top -b -n 1 | head -100") # RE
    ...
    basedme = ".singlepulse" # moved the quotes
    ...
    # RE command below
    os.system(" cp -rp %s /hercules/results/aswin/results/tests/workdirs/."%cwd )
